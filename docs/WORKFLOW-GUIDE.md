# OpenCode Game Studios -- Complete Workflow Guide
...
Run `/setup-engine` in OpenCode. This is the single most important
...
If you see this, hooks are working. If not, check `.opencode/opencode.json` to
...
.opencode/docs/templates/game-pillars.md
```
...
.opencode/docs/templates/game-design-document.md
```

**The `design-docs` rule** (`.opencode/rules/design-docs.md`) requires every GDD
...
The `narrative` rule (`.opencode/rules/narrative.md`) enforces:
...
.opencode/docs/templates/milestone-definition.md
```
...
**Key rule:** The `prototype-code` rule (`.opencode/rules/prototype-code.md`)
...
The `test-standards` rule (`.opencode/rules/test-standards.md`) enforces:
